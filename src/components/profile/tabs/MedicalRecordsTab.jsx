...
function MedicalRecordsTab() {
  ...
  useEffect(() => {
    const fetchUserDataAndResults = async () => {
      try {
        ...
        const userTestResults = allTestResults.filter(
          (test) => test.customerId === currentUserId
        );
  ...
  return (
    <div className="space-y-6">
      {/* STI Testing History Section */}
      <STITestingHistory />

      {/* General Test Results Section */}
      <div className="bg-white rounded-lg shadow p-6">
        ...
        {testResults.length === 0 ? (
          ...
          <div className="text-center py-8 bg-gray-50 rounded-lg">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Chưa có kết quả xét nghiệm khác
            </p>
            <p className="text-sm text-gray-500">
              Kết quả xét nghiệm khác của bạn sẽ được hiển thị sau khi bạn sử
              dụng dịch vụ
            </p>
          </div>
        ) : (
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {testResults.map((test) => {
                  ...
                  let statusText = "Đang chờ kết quả";

                  if (test.result === "positive") {
                    statusClass = "bg-red-100 text-red-800";
                    statusText = "Dương tính";
                  ...
                  return (
                    <tr key={test.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {test.examinedAt
                          ? new Date(test.examinedAt).toLocaleString("vi-VN", {
                              year: "numeric",
                              month: "2-digit",
                              day: "2-digit",
                              hour: "2-digit",
